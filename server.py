...
def _send_health_page(conn):
    ...
    body = (
        ...
        "<script>"
        "(function(){"
        "function lg(m){console.log('[bridge]',m);try{parent.postMessage('__BRIDGE_LOG__,'+m,'*')}catch(e){}}"
        "lg('script start');"
        ...
        "function c(){"
        "lg('creating ws');"
        "try{w=new WebSocket(u);lg('ws created ('+w.readyState+')')}catch(e){lg('ws error: '+e.message)}"
        "w.onopen=function(){lg('open');parent.postMessage('__BRIDGE_OPEN__','*')};"
        "w.onclose=function(e){lg('close code='+e.code);parent.postMessage('__BRIDGE_CLOSE__,'+e.code+','+(e.reason||'')+','+e.wasClean,'*');w=null};"
        "w.onerror=function(){lg('error');parent.postMessage('__BRIDGE_ERROR__','*')};"
